...
async def create_product(mocker):
    ...
    mock_product = ProductResponseSchema(id=1, **product_data)
    mock_repo = mocker.MagicMock(ProductRepository)
    mock_repo.add_one = AsyncMock(return_value=mock_product)
...
async def test_update_product(mocker):
    ...
    mock_repo = mocker.MagicMock(ProductRepository)
    mock_repo.get_one = AsyncMock(return_value=original_product)

    updated_product = original_product
    updated_product["name"] = "Updated Product Name"

    mock_repo.update_one = AsyncMock(return_value=updated_product)
    service = ProductService(product_repo=mock_repo)
    updated_product = await service.update(product_id=1, data=update_data)
